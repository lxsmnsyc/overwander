...
/**
 * The wall a sheet lends against one it is asked for: the one of the
 * same name, else the one the sheet names as its own, else its first.
 * A sheet's alternates are named alike, so a borrower's `wall-alt-1`
 * takes the lender's rather than its main outcrop three times over
 */
function lentFor(sheet: Sheet, name: unknown): Described | undefined {
  ...
  return (
    walls.find((one) => one.name === name) ?? walls.find((one) => one.name === named) ?? walls.at(0)
  );
}
...
for (const biome of biomes) {
  ...
  // Every wall that says it borrowed, not just the one the biome draws
  // with: a borrower whose alternates keep their old rock has five
  // cases of the wrong stone in the middle of the new
  const borrowing = target.terrains.filter(
    (one) => one.role === 'wall' && typeof one.borrowed === 'number',
  );

  if (stands == null || borrowing.length === 0) {
    continue;
  }
  const donors = new Map<number, Sheet>();
  const done: string[] = [];
  let repainted = 0;

  for (const takes of borrowing) {
    const lender = Number(takes.borrowed);
    const donor = donors.get(lender) ?? read(lender);

    donors.set(lender, donor);

    const lends = lentFor(donor, takes.name);
    ...
    if (laid > covered * MOSTLY_FLOOR) {
      throw new Error(
        `Biome ${lender}'s wall reads as ${Math.round((laid / covered) * 100)}% floor, so it cannot be lent`,
      );
    }
    takes.palette = -1;
    takes.missing = Array.isArray(lends.missing) ? lends.missing.map(Number) : [];
    done.push(`${String(takes.name)} <- ${lender}`);
    repainted += laid;
  }
  ...
  console.log(`${biome}: ${done.join(', ')}, ${repainted} pixels of floor repainted`);
}
