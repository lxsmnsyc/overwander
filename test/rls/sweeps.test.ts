...
describe('the rotation board', () => {
  ...
  it('leaves a window ahead of the player alone', async () => {
    // A zone is the player's own to report, so a day they have already
    // been on is a day they can come back to: sweeping it would hand
    // them a claim they have taken
    const tomorrow = dailyWindow(NOW + DAY);

    await sql`
      insert into rotation_claims (player, window_key, slot, claimed_at)
      values (${player.uid}, ${tomorrow}, 0, 1000)
    `;

    await listRotations(player.uid, NOW, 0);

    expect(await windows('rotation_claims')).toContain(tomorrow);
  });
