...
export default function PmdForm(): JSX.Element {
  ...
  // Either pair is enough. A female sheet is patched on its own, so
  // asking for the plain archives to do it would mean packing the
  // plain sheet again to change nothing about it
  const ready = (): boolean =>
    picked().some((name) => name === 'regular' || name === 'female') &&
    naming().species != null &&
    !packing.pending;
  ...
  return (
    ...
    >
      ...
      <FormSection
        title="PMD archives"
        lede="One archive a coat, as the collab site hands them out. A coat and its shiny are
          packed together; the female pair is packed on its own, and either can be given without
          the other."
      >
        <FormGrid>
          <For each={COATS}>
            {(coat) => (
              <FilePicker
                ...
                onPick={(files) => {
                  setPicked((names) =>
                    files.length > 0
                      ? [...names.filter((name) => name !== coat.name), coat.name]
                      : names.filter((name) => name !== coat.name),
                  );
                }}
              />
            )}
        ...
        <FormActions note="Writes a sheet and a description for each pair given.">
          ...
          {(done) => (
            <Written
              ...
              note={`${done().anims.length} animations kept, ${done().coats.length} ${
                ...
                .sheets.map(
                  (sheet) =>
                    `${sheet.female ? 'Female' : 'Plain'} ${sheet.width} × ${sheet.height}`,
                )
                .join(', ')}.`}
            />
          )}
