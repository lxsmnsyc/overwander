import { TrainerClass } from './classes';

/**
 * The charsets a class may be standing in, rolled per stop the way a
 * wanderer's style is
 */
const TRAINER_CHARSETS: Record<TrainerClass, string[]> = {
  [TrainerClass.AceTrainer]: [
    'characters/frlg/ace-trainer-f',
    'characters/frlg/ace-trainer-m',
    'characters/lgpe/ace-trainer',
  ],
  [TrainerClass.Lass]: ['characters/frlg/lass', 'characters/lgpe/lass'],
  [TrainerClass.BlackBelt]: ['characters/lgpe/black-belt', 'characters/frlg/crush-girl'],
  [TrainerClass.BirdKeeper]: ['characters/lgpe/bird-keeper'],
  [TrainerClass.Biker]: ['characters/frlg/roughneck', 'characters/lgpe/punk'],
  [TrainerClass.Hiker]: ['characters/frlg/hiker', 'characters/lgpe/hiker'],
  [TrainerClass.PokeManiac]: ['characters/lgpe/poke-maniac', 'characters/frlg/ruin-maniac'],
  [TrainerClass.BugCatcher]: ['characters/frlg/bug-catcher', 'characters/lgpe/bug-catcher'],
  [TrainerClass.Channeler]: ['characters/lgpe/channeler'],
  [TrainerClass.Burglar]: ['characters/lgpe/burglar'],
  [TrainerClass.Swimmer]: ['characters/lgpe/swimmer-f', 'characters/lgpe/swimmer-m'],
  [TrainerClass.Rocker]: ['characters/frlg/rocker', 'characters/lgpe/rocker'],
  [TrainerClass.Psychic]: ['characters/lgpe/psychic'],
  // The tower's sages and the two elders who keep it, who are the
  // same people a few decades apart
  [TrainerClass.Sage]: [
    'characters/hgss/sage',
    'characters/hgss/sage-1',
    'characters/hgss/sage-2',
    'characters/hgss/elder-1',
    'characters/hgss/elder-2',
  ],
  [TrainerClass.Skier]: ['characters/hgss/skier'],
  [TrainerClass.Scientist]: ['characters/hgss/scientist', 'characters/lgpe/scientist'],
  [TrainerClass.JohtoPokeManiac]: ['characters/hgss/poke-maniac'],
  [TrainerClass.JohtoBurglar]: ['characters/hgss/burglar'],
  [TrainerClass.JohtoAceTrainer]: ['characters/hgss/ace-trainer', 'characters/hgss/ace-trainer-f'],
  [TrainerClass.JohtoLass]: ['characters/hgss/lass'],
  [TrainerClass.JohtoBlackBelt]: ['characters/hgss/black-belt'],
  [TrainerClass.JohtoBirdKeeper]: ['characters/hgss/bird-keeper'],
  [TrainerClass.JohtoBiker]: ['characters/hgss/biker'],
  [TrainerClass.JohtoBugCatcher]: ['characters/hgss/bug-catcher'],
  [TrainerClass.JohtoSwimmer]: ['characters/hgss/swimmer', 'characters/hgss/swimmer-f'],
  [TrainerClass.Firebreather]: ['characters/hgss/firebreather'],
  [TrainerClass.Medium]: ['characters/hgss/medium'],
  [TrainerClass.Teacher]: ['characters/hgss/teacher'],
  [TrainerClass.SchoolKid]: ['characters/hgss/school-kid'],
  [TrainerClass.Youngster]: ['characters/hgss/youngster'],
  // The two halves of one trade, the way the Black Belt keeps the
  // Crush Girl's sheet
  [TrainerClass.Camper]: ['characters/lgpe/camper', 'characters/lgpe/picnicker'],
  [TrainerClass.Beauty]: ['characters/frlg/beauty'],
  [TrainerClass.Fisherman]: ['characters/lgpe/fisherman'],
  [TrainerClass.Sailor]: ['characters/frlg/sailor', 'characters/lgpe/sailor'],
  [TrainerClass.Gentleman]: ['characters/frlg/gentleman', 'characters/lgpe/gentleman'],
  [TrainerClass.SuperNerd]: ['characters/lgpe/super-nerd'],
  [TrainerClass.Juggler]: ['characters/lgpe/juggler'],
  [TrainerClass.Tamer]: ['characters/frlg/tamer', 'characters/lgpe/tamer'],
  [TrainerClass.Engineer]: ['characters/lgpe/engineer'],
  [TrainerClass.Gambler]: ['characters/lgpe/gambler'],
  [TrainerClass.JohtoGentleman]: ['characters/hgss/gentleman', 'characters/hgss/gentleman-2'],
  [TrainerClass.JohtoSuperNerd]: ['characters/hgss/super-nerd'],
  [TrainerClass.JohtoJuggler]: ['characters/hgss/juggler'],
  [TrainerClass.Boarder]: ['characters/hgss/boarder'],
  [TrainerClass.NinjaBoy]: ['characters/oras/ninja-boy', 'characters/rse/ninja-boy'],
  [TrainerClass.Tuber]: ['characters/oras/tuber-f', 'characters/oras/tuber-m'],
  [TrainerClass.PokeFan]: ['characters/oras/pokefan-f', 'characters/oras/pokefan-m'],
  [TrainerClass.HoennAceTrainer]: ['characters/oras/ace-trainer', 'characters/oras/ace-trainer-f'],
  [TrainerClass.HoennLass]: ['characters/oras/lass'],
  [TrainerClass.HoennBirdKeeper]: ['characters/oras/bird-keeper'],
  [TrainerClass.HoennBugCatcher]: ['characters/oras/bug-catcher'],
  [TrainerClass.HoennSwimmer]: ['characters/oras/swimmer'],
  [TrainerClass.HoennYoungster]: ['characters/oras/youngster'],
  [TrainerClass.HoennSchoolKid]: ['characters/oras/schoolkid-f', 'characters/oras/schoolkid-m'],
  [TrainerClass.HoennCamper]: ['characters/oras/camper', 'characters/oras/picnicker'],
  [TrainerClass.HoennBeauty]: ['characters/oras/beauty'],
  [TrainerClass.HoennFisherman]: ['characters/oras/fisher'],
  [TrainerClass.HoennSailor]: ['characters/oras/sailor'],
  [TrainerClass.HoennGentleman]: ['characters/oras/gentleman'],
  [TrainerClass.HoennScientist]: ['characters/rse/devon-researcher'],
  [TrainerClass.Guitarist]: ['characters/oras/guitarist'],
  [TrainerClass.Kindler]: ['characters/oras/kindler'],
  [TrainerClass.BattleGirl]: ['characters/oras/battle-girl'],
  [TrainerClass.Expert]: ['characters/oras/expert'],
  [TrainerClass.RuinManiac]: ['characters/oras/ruin-maniac'],
  [TrainerClass.StreetThug]: ['characters/oras/street-thug'],
  [TrainerClass.DragonTamer]: ['characters/oras/dragon-tamer'],
  [TrainerClass.AromaLady]: ['characters/oras/aroma-lady'],
  [TrainerClass.Ranger]: ['characters/dppt/ranger-f', 'characters/dppt/ranger-m'],
  [TrainerClass.Worker]: ['characters/dppt/worker'],
  [TrainerClass.Rancher]: ['characters/dppt/rancher', 'characters/dppt/cowgirl'],
  [TrainerClass.Cyclist]: ['characters/dppt/cyclist', 'characters/dppt/cyclist-f'],
  [TrainerClass.PokeKid]: ['characters/dppt/poke-kid'],
  [TrainerClass.Collector]: ['characters/dppt/collector'],
  [TrainerClass.Artist]: ['characters/dppt/artist'],
  [TrainerClass.Reporter]: ['characters/dppt/reporter', 'characters/dppt/cameraman'],
  [TrainerClass.RichBoy]: ['characters/dppt/rich-boy', 'characters/dppt/lady'],
  [TrainerClass.Waiter]: ['characters/dppt/waiter', 'characters/dppt/waitress'],
  [TrainerClass.ParasolLady]: ['characters/dppt/parasol-lady'],
  [TrainerClass.Twins]: ['characters/dppt/twins'],
  [TrainerClass.Policeman]: ['characters/dppt/policeman'],
  [TrainerClass.Jogger]: ['characters/dppt/jogger'],
  [TrainerClass.Breeder]: ['characters/dppt/breeder-f', 'characters/dppt/breeder-m'],
  [TrainerClass.SinnohAceTrainer]: [
    'characters/dppt/ace-trainer-f',
    'characters/dppt/ace-trainer-f-2',
    'characters/dppt/ace-trainer-m',
    'characters/dppt/ace-trainer-m-2',
  ],
  [TrainerClass.SinnohLass]: ['characters/dppt/lass'],
  [TrainerClass.SinnohBugCatcher]: ['characters/dppt/bug-catcher'],
  [TrainerClass.SinnohSwimmer]: ['characters/dppt/swimmer-f', 'characters/dppt/swimmer-m'],
  [TrainerClass.SinnohYoungster]: ['characters/dppt/youngster'],
  [TrainerClass.SinnohSchoolKid]: ['characters/dppt/schoolkid-f', 'characters/dppt/schoolkid-m'],
  [TrainerClass.SinnohCamper]: ['characters/dppt/camper', 'characters/dppt/picnicker'],
  [TrainerClass.SinnohBeauty]: ['characters/dppt/beauty', 'characters/dppt/idol'],
  [TrainerClass.SinnohFisherman]: ['characters/dppt/fisher'],
  [TrainerClass.SinnohSailor]: ['characters/dppt/sailor'],
  [TrainerClass.SinnohGentleman]: ['characters/dppt/gentleman', 'characters/dppt/socialite'],
  [TrainerClass.SinnohScientist]: ['characters/dppt/scientist'],
  [TrainerClass.SinnohBirdKeeper]: ['characters/dppt/bird-keeper'],
  [TrainerClass.SinnohHiker]: ['characters/dppt/hiker'],
  [TrainerClass.SinnohPsychic]: [
    'characters/dppt/psychic-f',
    'characters/dppt/psychic-m',
    'characters/dppt/expert',
  ],
  [TrainerClass.SinnohSkier]: ['characters/dppt/skier-f', 'characters/dppt/skier-m'],
  [TrainerClass.SinnohBlackBelt]: ['characters/dppt/black-belt', 'characters/dppt/battle-girl'],
  [TrainerClass.SinnohDragonTamer]: ['characters/dppt/dragon-tamer'],
  [TrainerClass.SinnohGuitarist]: ['characters/dppt/guitarist'],
  [TrainerClass.SinnohAromaLady]: ['characters/dppt/aroma-lady'],
  [TrainerClass.SinnohRuinManiac]: ['characters/dppt/ruin-maniac'],
  [TrainerClass.SinnohNinjaBoy]: ['characters/dppt/ninja-boy'],
  [TrainerClass.SinnohTuber]: ['characters/dppt/tuber-f', 'characters/dppt/tuber-m'],
  [TrainerClass.SinnohPokeFan]: ['characters/dppt/poke-fan-f', 'characters/dppt/poke-fan-m'],
  [TrainerClass.SinnohRoughneck]: ['characters/dppt/roughneck'],
  [TrainerClass.SinnohClown]: ['characters/dppt/clown', 'characters/dppt/clown-f'],
  [TrainerClass.Couple]: ['characters/oras/couple-f', 'characters/oras/couple-m'],
  [TrainerClass.HoennBreeder]: ['characters/oras/breeder-f', 'characters/oras/breeder-m'],
  [TrainerClass.HoennRanger]: ['characters/oras/ranger-f', 'characters/oras/ranger-m'],
  [TrainerClass.HoennCollector]: ['characters/oras/collector'],
  [TrainerClass.HoennReporter]: ['characters/oras/reporter', 'characters/oras/cameraman'],
  [TrainerClass.HoennRichBoy]: ['characters/oras/rich-boy', 'characters/oras/lady'],
  [TrainerClass.HoennParasolLady]: ['characters/oras/parasol-lady'],
  [TrainerClass.SinnohCouple]: ['characters/dppt/couple-f', 'characters/dppt/couple-m'],
  [TrainerClass.Backpacker]: ['characters/b2w2/backpacker-f', 'characters/b2w2/backpacker-m'],
  [TrainerClass.Baker]: ['characters/b2w2/baker'],
  [TrainerClass.Clerk]: [
    'characters/b2w2/clerk-f',
    'characters/b2w2/clerk-m-1',
    'characters/b2w2/clerk-m-2',
  ],
  [TrainerClass.Dancer]: ['characters/b2w2/dancer'],
  [TrainerClass.DepotAgent]: ['characters/b2w2/depot-agent'],
  [TrainerClass.Doctor]: ['characters/b2w2/doctor'],
  [TrainerClass.Harlequin]: ['characters/b2w2/harlequin'],
  [TrainerClass.Hoopster]: ['characters/b2w2/hoopster'],
  [TrainerClass.Infielder]: ['characters/b2w2/infielder'],
  [TrainerClass.Janitor]: ['characters/b2w2/janitor'],
  [TrainerClass.Lady]: ['characters/b2w2/lady'],
  [TrainerClass.Linebacker]: ['characters/b2w2/linebacker'],
  [TrainerClass.Maid]: ['characters/b2w2/maid'],
  [TrainerClass.Musician]: ['characters/b2w2/musician'],
  [TrainerClass.NurseryAide]: ['characters/b2w2/nursery-aide'],
  [TrainerClass.Pilot]: ['characters/b2w2/pilot'],
  [TrainerClass.Smasher]: ['characters/b2w2/smasher'],
  [TrainerClass.Socialite]: ['characters/b2w2/socialite'],
  [TrainerClass.Striker]: ['characters/b2w2/striker'],
  [TrainerClass.SuitActor]: ['characters/b2w2/suit-actor'],
  [TrainerClass.Veteran]: ['characters/b2w2/veteran'],
  [TrainerClass.Backers]: ['characters/b2w2/backers-1', 'characters/b2w2/backers-2'],
  [TrainerClass.UnovaAceTrainer]: [
    'characters/b2w2/ace-trainer-f',
    'characters/b2w2/ace-trainer-m',
  ],
  [TrainerClass.UnovaArtist]: ['characters/b2w2/artist'],
  [TrainerClass.UnovaBattleGirl]: ['characters/b2w2/battle-girl'],
  [TrainerClass.UnovaBeauty]: ['characters/b2w2/beauty'],
  [TrainerClass.UnovaBiker]: ['characters/b2w2/biker'],
  [TrainerClass.UnovaBlackBelt]: ['characters/b2w2/black-belt'],
  [TrainerClass.UnovaBreeder]: ['characters/b2w2/breeder-f', 'characters/b2w2/breeder-m'],
  [TrainerClass.UnovaCyclist]: ['characters/b2w2/cyclist-f', 'characters/b2w2/cyclist-m'],
  [TrainerClass.UnovaFisherman]: ['characters/b2w2/fisher'],
  [TrainerClass.UnovaGentleman]: ['characters/b2w2/gentleman'],
  [TrainerClass.UnovaGuitarist]: ['characters/b2w2/guitarist'],
  [TrainerClass.UnovaHiker]: ['characters/b2w2/hiker'],
  [TrainerClass.UnovaLass]: ['characters/b2w2/lass'],
  [TrainerClass.UnovaParasolLady]: ['characters/b2w2/parasol-lady'],
  [TrainerClass.UnovaPokeFan]: ['characters/b2w2/poke-fan-f', 'characters/b2w2/poke-fan-m'],
  [TrainerClass.UnovaPoliceman]: ['characters/b2w2/policeman'],
  [TrainerClass.UnovaPsychic]: ['characters/b2w2/psychic-f', 'characters/b2w2/psychic-m'],
  [TrainerClass.UnovaRanger]: ['characters/b2w2/ranger-f', 'characters/b2w2/ranger-m'],
  [TrainerClass.UnovaRichBoy]: ['characters/b2w2/rich-boy'],
  [TrainerClass.UnovaRoughneck]: ['characters/b2w2/roughneck'],
  [TrainerClass.UnovaSchoolKid]: ['characters/b2w2/schoolkid-f', 'characters/b2w2/schoolkid-m'],
  [TrainerClass.UnovaScientist]: ['characters/b2w2/scientist-f', 'characters/b2w2/scientist-m'],
  [TrainerClass.UnovaSwimmer]: ['characters/b2w2/swimmer-f', 'characters/b2w2/swimmer-m'],
  [TrainerClass.UnovaWaiter]: ['characters/b2w2/waiter-f', 'characters/b2w2/waiter-m'],
  [TrainerClass.UnovaWorker]: ['characters/b2w2/worker-1', 'characters/b2w2/worker-2'],
  [TrainerClass.UnovaYoungster]: ['characters/b2w2/youngster'],
};

export default TRAINER_CHARSETS;
