...
function tended(
  ...
  // being. Only a purified one gains the values: filling anything else
  // to the raised pool left it holding more health than it has
  const filled = getMaxHealth(
    purified == null ? record : { ...record, ivs: purifyIVs(record.ivs) },
  );
