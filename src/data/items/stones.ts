...
const LATENT_STONES: [item: Items, name: string, icon: string][] = [
  [Items.SunStone, 'Sun Stone', 'sun-stone'],
 ...
 * evolution.
 *
 * The five Kanto stones are stocked and found; the five below them
 * are neither yet. Every line that asks for a Sun Stone or an Ice
 * Stone belongs to a generation this game has not registered, so a
 * vendor selling one would be selling a stone with nothing to spend
 * it on. They are registered anyway — a name, a picture and a price
 * are what an item is, and the day a Gloom can become a Bellossom the
 * only change needed is a line in the pool
 */
