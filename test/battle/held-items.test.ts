...
describe('Meteorite', () => {
  it('does nothing at all in a hand during a fight', () => {
    const { battle, teamA } = createBattle();
    ...
    // The rock is used on it from the catch sheet now, not carried
    // into a fight: the shape a Deoxys walks in as is the one it
    // fights in
    expect(deoxys.species).toBe(Species.Deoxys);
  });
});
