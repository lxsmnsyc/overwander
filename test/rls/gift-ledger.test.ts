...
import { QUEST_GIFT, giftId, listAllGifts } from '../../src/server/gifts';
 ...
 * database.
 *
 * Quest rewards are paid through the gift rows, one per quest per
 * player, so a live game writes far more of them than staff ever will.
 * The ledger leaves them out, and the rule is a prefix on the id —
...
describe('the dashboard gift ledger', () => {
  it('leaves quest rewards out and keeps everything else', async () => {
    await put(giftId('staff-abc123', player.uid), 'By hand.');
    await put(giftId(`${QUEST_GIFT}7-1000`, player.uid), 'Quest: First Steps.');
    await put(giftId('rotation-3-0-1000', player.uid), 'This window.');
    await put('open-for-everybody', 'Open to all.');

    expect(await reasons()).toEqual(['By hand.', 'Open to all.', 'This window.']);
  });
  ...
  it('does not catch a gift that merely mentions a quest', async () => {
    // The rule is the id, not the sentence: staff writing "quest" in a
    // reason must not vanish from their own ledger
    await put(giftId('staff-xyz', player.uid), 'Quest: written by hand as a make-good.');

    expect(await reasons()).toEqual(['Quest: written by hand as a make-good.']);
  });
});
